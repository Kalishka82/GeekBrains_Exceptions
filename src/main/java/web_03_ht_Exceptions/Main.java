...
public class Main {
    ...
    public static void makeRecord() throws Exception{
        System.out.println("Введите через пробел: фамилию, имя, отчество, дату рождения (dd.mm.yyyy), " +
                "номер телефона (число без разделителей) и пол (f или m)");
        ...
        if (array.length != 6){
            throw new Exception("Введено неверное кол-во параметров");
        }

        String surname = array[0];
        String name = array[1];
        String midName = array[2];
        ...
        try (FileWriter fileWriter = new FileWriter(file, true)){
            ...
            fileWriter.write(String.format("%s %s %s %s %s %s", surname, name, midName, format.format(birthdate), phone, sex));
        } catch (IOException e) {
